...
const CAT_CFG = {
  Drink:    { bg: '#EBF3FF', color: '#1D5FCC', dot: '#3A8EF6', label: 'Voucher' },
};
...
const compressImageToWebP = (file: File, maxWidth = 800, maxHeight = 800, quality = 0.8): Promise<Blob> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (!ctx) { reject(new Error("Failed to initialize Canvas API")); return; }
      ctx.drawImage(img, 0, 0, width, height);
      canvas.toBlob((blob) => { if (blob) resolve(blob); else reject(new Error("Failed to convert image")); }, "image/webp", quality);
    };
...
const globalStyles = `
  ...
  .gc-select { appearance: none !important; background-image: url("data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1L5 5L9 1' stroke='%238C91AC' stroke-width='1.4' stroke-linecap='round'/%3E%3C/svg%3E") !important; background-repeat: no-repeat !important; background-position: right 12px center !important; padding-right: 32px !important; }
`;
...
function LiveBadge({ status }: { status: SyncStatus }) {
  ...
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontSize:11.5, fontWeight:600, color, fontFamily:font }}>
      <span style={{ width:6, height:6, borderRadius:'50%', background:color, flexShrink:0, animation: status==='live' ? 'pulseDot 2s ease-in-out infinite' : 'none', boxShadow: status==='live' ? `0 0 0 2px ${color}30` : 'none' }}/>
      {label}
    </span>
  );
}
...
function Toast({ msg, type, onDone }: { msg:string; type:'success'|'error'; onDone:()=>void }) {
  ...
  return <div style={{ position:'fixed', bottom:28, right:28, zIndex:999 }}><GcToast msg={msg} type={type} /></div>;
}
...
function StatCard({ label, value, color, bg, icon }: { label:string; value:number; color:string; bg:string; icon:React.ReactNode }) {
  return (
    <div className="gc-stat-card" style={{ background:C.white, borderRadius:16, padding:'16px 20px', border:`1px solid ${C.border}`, display:'flex', alignItems:'center', gap:14 }}>
      <div style={{ width:44, height:44, borderRadius:12, background:bg, color:color, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{icon}</div>
      <div>
...
function DeleteModal({ reward, onClose, onDeleted }: { reward:Reward; onClose:()=>void; onDeleted:(msg:string)=>void }) {
  ...
  return (
    <GcModalShell onClose={onClose} title="Delete this reward?" eyebrow="Destructive Action" description={<><strong style={{ color:C.tx1 }}>"{reward.title}"</strong> will be permanently deleted from Firestore.</>} maxWidth={420} footer={<><GcButton variant="ghost" size="lg" onClick={onClose}>Cancel</GcButton><GcButton variant="danger" size="lg" onClick={confirm} loading={loading}>Yes, Delete</GcButton></>}>
      <code style={{ fontSize:10.5, color:C.tx3, background:C.bg, padding:'4px 9px', borderRadius:6, display:'inline-block', marginBottom:20, fontFamily:font, border:`1px solid ${C.border}` }}>{reward.id}</code>
...
function RewardModal({ reward, onClose, onSaved, onDeleteRequest }: { reward:Reward|null; onClose:()=>void; onSaved:(msg:string)=>void; onDeleteRequest?:(r:Reward)=>void }) {
  const isNew = !reward;
  ...
  const set = (k:keyof RewardForm) => (e:React.ChangeEvent<HTMLInputElement|HTMLTextAreaElement|HTMLSelectElement>) => setForm(p => ({ ...p, [k]:e.target.value }));
  ...
  const toggleStorageLibrary = useCallback(async () => {
    const next = !showLibrary; setShowLibrary(next);
  ...
  return (
    <GcModalShell onClose={onClose} title={isNew ? 'Add Voucher' : reward!.title} eyebrow={isNew ? 'New Reward' : 'Edit Reward'} description={!isNew ? <code style={{ fontSize:10.5, color:C.tx3, background:C.bg, padding:'2px 8px', borderRadius:5, fontFamily:font, border:`1px solid ${C.border}`, display:'inline-block' }}>{reward!.id}</code> : undefined} maxWidth={540} footer={
      <>
        {!isNew && onDeleteRequest && (
          <div style={{ marginRight: 'auto' }}>
            <GcButton variant="danger" size="lg" onClick={() => { onClose(); onDeleteRequest(reward!); }}>Delete</GcButton>
          </div>
        )}
        <div style={{ marginLeft: !isNew && onDeleteRequest ? 0 : 'auto', display: 'flex', gap: 8 }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              {form.imageUrl && <img src={form.imageUrl} alt={form.title} style={{ width: '100%', height: '100%', objectFit: 'contain', maxWidth: 44, maxHeight: 44, borderRadius: 8, border: `1px solid ${C.border}` }} />}
              <div style={{ flex: 1 }}>
                {processingImage ? <div style={{ width: '100%', height: 42, borderRadius: 9, background: C.bg, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontSize: 12, fontWeight: 700, color: C.orange }}>Compressing to WebP...</span></div>
                : uploadProgress !== null ? <div style={{ width: '100%', height: 42, borderRadius: 9, background: C.bg, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', padding: '0 10px', position: 'relative', overflow: 'hidden' }}><div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, background: C.blueL, width: `${uploadProgress}%`, transition: 'width 0.2s ease' }} /><span style={{ position: 'relative', zIndex: 1, fontSize: 12, fontWeight: 700, color: C.blue }}>Uploading... {Math.round(uploadProgress)}%</span></div>
                : <div style={{ position: 'relative', height: 42, display: 'flex', alignItems: 'center' }}><input type="file" accept="image/*" onChange={handleImageFile} style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer', zIndex: 10 }} /><div style={{ width: '100%', height: '100%', borderRadius: 9, background: C.bg, border: `1.5px dashed ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 12.5, fontWeight: 600, color: C.tx2, transition: 'all .2s' }}><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>{form.imageUrl ? "Change Voucher Image" : "Select Image File"}</div></div>}
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
              <GcButton type="button" variant="ghost" size="sm" onClick={toggleStorageLibrary} disabled={processingImage || uploadProgress !== null}>{showLibrary ? 'Hide Storage Gallery' : 'Choose from Storage'}</GcButton>
              {showLibrary && <GcButton type="button" variant="ghost" size="sm" onClick={loadStorageLibrary} disabled={libraryLoading}>{libraryLoading ? 'Loading…' : 'Refresh'}</GcButton>}
            </div>
            {showLibrary && (
              <div style={{ marginTop: 10, border: `1px solid ${C.border}`, borderRadius: 11, background: C.white, overflow: 'hidden' }}>
                <div style={{ padding: '9px 11px', borderBottom: `1px solid ${C.border2}`, fontSize: 11.5, fontWeight: 700, color: C.tx2 }}>Firebase Storage /rewards ({libraryImages.length})</div>
                {libraryLoading ? <div style={{ padding: '14px 12px', fontSize: 12, color: C.tx3 }}>Loading gallery…</div> : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(92px, 1fr))', gap: 8, padding: 10, maxHeight: 220, overflowY: 'auto' }}>
                    {libraryImages.map((img) => {
                      ...
                      return (
                        <button key={img.path} type="button" title={img.name} onClick={() => setForm(p => ({ ...p, imageUrl: img.url }))} style={{ border: selected ? `2px solid ${C.blue}` : `1px solid ${C.border}`, borderRadius: 9, background: selected ? C.blueL : C.bg, cursor: 'pointer', padding: 4, display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'stretch' }}>
  ...
  return (
    <div className="gc-ticket" onClick={onEdit} style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, boxShadow: C.shadow, display: 'flex', overflow: 'visible', position: 'relative', cursor: 'pointer' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '14px 16px 12px 20px', minWidth: 0 }}>
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:8, marginBottom:5 }}><p style={{ fontSize:13.5, fontWeight:700, color:C.tx1, lineHeight:1.3, letterSpacing:'-.015em', fontFamily:font, margin:0, flex:1, minWidth:0 }}>{reward.title}</p></div>
        <p style={{ fontSize:12, color:C.tx3, lineHeight:1.55, fontFamily:font, margin:0, marginBottom:10, flex:1, overflow:'hidden', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' as any }}>{reward.description || <span style={{ fontStyle:'italic', color:C.tx4 }}>No description.</span>}</p>
        <div style={{ display:'flex', alignItems:'center', gap:7, flexWrap:'wrap' }}>
          {reward.imageUrl && <div style={{ width:22, height:22, borderRadius:4, overflow:'hidden', border:`1px solid ${C.border2}`, flexShrink:0 }}><img src={reward.imageUrl} alt={reward.title} style={{ width:'100%', height:'100%', objectFit:'contain' }}/></div>}
          <code style={{ fontSize:9.5, color:C.tx4, background:C.bg, padding:'2px 7px', borderRadius:5, fontFamily:font, border:`1px solid ${C.border2}`, flexShrink:0 }}>{reward.id}</code>
          <div style={{ flex:1 }}/>
        </div>
      </div>
    </div>
...
function AddTicket({ onClick }: { onClick: () => void }) {
  return (
    <div onClick={onClick} className="gc-add-ticket" style={{ border:`1.5px dashed ${C.border}`, borderRadius:16, background:'transparent', display:'flex', alignItems:'center', justifyContent:'center', height:88, cursor:'pointer', transition:'all .18s', gap:12 }}>
      <div style={{ width:30, height:30, borderRadius:9, background:C.bgSub, display:'flex', alignItems:'center', justifyContent:'center', color:C.tx3, transition:'all .18s', flexShrink:0 }}><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
      <div><p style={{ fontSize:12.5, fontWeight:600, color:C.tx3, margin:0, fontFamily:font }}>Add New Voucher</p><p style={{ fontSize:11, color:C.tx4, margin:'2px 0 0', fontFamily:font }}>Click to add reward</p></div>
    </div>
  );
}
...
function ShowcaseRewardCard({ reward }: { reward: Reward }) {
  ...
  return (
    <div onMouseOver={() => setH(true)} onMouseOut={() => setH(false)} style={{ background: C.white, borderRadius: 16, border: `1px solid ${h ? cat.color + '44' : C.border}`, boxShadow: h ? `0 8px 28px rgba(13,15,23,.10), 0 2px 6px rgba(13,15,23,.04)` : C.shadow, overflow: 'hidden', transition: 'all .18s ease', transform: h ? 'translateY(-3px)' : 'none', display: 'flex', flexDirection: 'column', cursor: 'default', height: '100%' }}>
      <div style={{ height: 160, background: img ? 'linear-gradient(135deg, #F4F6FB 0%, #E8EBF4 100%)' : cat.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', flexShrink: 0, overflow: 'hidden' }}>
        {img ? <img src={img} alt={reward.title} style={{ width: '100%', height: '100%', objectFit: 'contain', padding: 14 }} /> : <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, opacity: .7 }}><div style={{ textAlign: 'center' }}>{pts === 0 ? <p style={{ fontSize: 18, fontWeight: 800, color: C.green, letterSpacing: '-.02em', lineHeight: 1, fontFamily: font, margin: 0 }}>FREE</p> : <><p style={{ fontSize: pts >= 10000 ? 18 : 22, fontWeight: 800, color: cat.color, letterSpacing: '-.03em', lineHeight: 1, fontFamily: font, margin: 0 }}>{pts.toLocaleString('id')}</p><p style={{ fontSize: 10, fontWeight: 700, color: cat.color, opacity: .65, letterSpacing: '.09em', textTransform: 'uppercase', fontFamily: font, margin: '4px 0 0' }}>pts</p></>}</div></div>}
      </div>
      <div style={{ padding: '14px 16px 16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        <p style={{ fontSize: 14.5, fontWeight: 800, color: C.tx1, margin: 0, lineHeight: 1.3, fontFamily: font }}>{reward.title}</p>
        {reward.description && <p style={{ fontSize: 12, color: C.tx3, margin: 0, lineHeight: 1.55, fontFamily: font, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as any }}>{reward.description}</p>}
        <div style={{ marginTop: 'auto', paddingTop: 10, borderTop: `1px solid ${C.border2}`, display: 'flex', alignItems: 'baseline', gap: 6 }}>
          {pts === 0 ? <p style={{ fontSize: 16, fontWeight: 800, color: C.green, margin: 0, fontFamily: font }}>Gratis</p> : <><p style={{ fontSize: 16, fontWeight: 800, color: cat.color, margin: 0, fontFamily: font }}>{pts.toLocaleString('id')}</p><span style={{ fontSize: 11, color: C.tx3, fontWeight: 600, fontFamily: font }}>poin</span></>}
        </div>
      </div>
    </div>
  );
...
export default function RewardsClient({ initialRewards = [], showAddTrigger }: { initialRewards?: Reward[]; showAddTrigger?: boolean }) {
    ...
    return (
      <>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <GcButton variant="blue" size="lg" disabled={!canMutate} onClick={() => canMutate && setShowAdd(true)}>Add Voucher</GcButton>
        </div>
  ...
  if (!canMutate) {
    return (
      <GcPage>
        <style>{globalStyles}</style>
        <GcPageHeader eyebrow="Voucher Studio" title="Rewards & Voucher Catalog" description="Katalog reward dan voucher yang tersedia untuk ditukarkan oleh member." actions={<LiveBadge status={syncStatus}/>} />
        <div className="gc-grid-4" style={{ gap:12, marginBottom:22 }}>
          <StatCard label="Total Reward" value={total} color={C.blue} bg={C.blueL} icon={<svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 010-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 000-5C13 2 12 7 12 7z"/></svg>} />
          <StatCard label="Aktif" value={activeAmt} color={C.green} bg={C.greenBg} icon={<svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M20 6L9 17l-5-5"/></svg>} />
          <StatCard label="Nonaktif" value={inactive} color={C.red} bg={C.redBg} icon={<svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg>} />
        </div>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18, gap:10, flexWrap:'wrap' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
            <div className="gc-search" style={{ display:'flex', alignItems:'center', gap:8, height:36, padding:'0 12px', background:C.white, border:`1.5px solid ${C.border}`, borderRadius:9, transition:'all .15s', minWidth:200 }}>
              ...
              {search && <button onClick={()=>setSearch('')} style={{ background:'none', border:'none', cursor:'pointer', color:C.tx3, padding:0, lineHeight:1, display:'flex' }}><svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M1 1l10 10M11 1L1 11" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"/></svg></button>}
            </div>
          ...
          <span style={{ fontSize:12, color:C.tx3, fontFamily:font, fontWeight:500 }}>{filtered.length}{filtered.length!==rewards.length?` / ${rewards.length}`:''} reward</span>
        </div>
        {filtered.length === 0 ? (
          <GcPanel style={{ padding:'50px 24px', textAlign:'center' }}><GcEmptyState title="Tidak ada reward" description={syncStatus==='connecting'?'Memuat data…':'Tidak ada reward yang sesuai filter.'} icon={syncStatus==='connecting'?'⏳':'📭'} /></GcPanel>
        ) : (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(210px, 1fr))', gap:16 }}>
            {filtered.map((reward, i) => (
              <div key={reward.id} style={{ animation:`gcSlideUp .24s ease both`, animationDelay:`${i*25}ms` }}><ShowcaseRewardCard reward={reward} /></div>
            ))}
          </div>
        )}
        {toast && <Toast msg={toast.msg} type={toast.type} onDone={()=>setToast(null)}/>}
  ...
  return (
    <GcPage>
      ...
      <GcPageHeader
        ...
        actions={<div style={{ display:'flex', alignItems:'center', gap:12 }}><LiveBadge status={syncStatus}/>{canMutate && <GcButton variant="blue" size="lg" onClick={() => setShowAdd(true)}>Add Voucher</GcButton>}</div>}
      />
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18, gap:10, flexWrap:'wrap' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:14, flexShrink:0 }}><div style={{ display:'flex', alignItems:'center', gap:10 }}><span style={{ fontSize:12, color:C.tx3, fontFamily:font, fontWeight: 500 }}>{filtered.length}{filtered.length!==rewards.length?` / ${rewards.length}`:''} reward</span></div></div>
      </div>

      {rewards.length===0 && syncStatus!=='connecting' ? (
        <GcPanel style={{ padding:'70px 24px', textAlign:'center' }}><GcEmptyState title="No rewards yet" description="Start adding your first voucher." /><div style={{ marginTop: 22 }}><GcButton variant="blue" size="lg" disabled={!canMutate} onClick={()=>canMutate && setShowAdd(true)}>+ Add First Voucher</GcButton></div></GcPanel>
      ) : filtered.length===0 ? (
        <GcPanel style={{ padding:'50px 24px', textAlign:'center' }}><GcEmptyState title="No results" description={syncStatus==='connecting' ? 'Loading data…' : 'No rewards match this filter.'} icon={syncStatus==='connecting' ? '⏳' : '📭'} /></GcPanel>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {filtered.map((reward, i) => (
            <div key={reward.id} style={{ animation:`gcSlideUp .24s ease both`, animationDelay:`${i*30}ms` }}>
              <RewardCard reward={reward} onEdit={() => canMutate && setEditTarget(reward)} />
            </div>
          ))}
          <div style={{ animation:`gcSlideUp .24s ease both`, animationDelay:`${filtered.length*30}ms` }}>{canMutate && <AddTicket onClick={()=>setShowAdd(true)}/>}</div>
        </div>
      )}

      {canMutate && editTarget && <RewardModal reward={editTarget} onClose={()=>setEditTarget(null)} onSaved={msg=>{showToast(msg);setEditTarget(null);}} onDeleteRequest={setDeleteTarget} />}
