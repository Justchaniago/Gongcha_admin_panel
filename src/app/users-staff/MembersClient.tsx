...
function FL({ children }: { children: React.ReactNode }) {
  return <label style={{ display: 'block', marginBottom: 6, fontSize: 11, fontWeight: 700,
    letterSpacing: '.06em', textTransform: 'uppercase', color: C.tx3 }}>{children}</label>;
}

function GcInput({ style, ...p }: React.InputHTMLAttributes<HTMLInputElement>) {
  const [f, setF] = useState(false);
  return <input {...p} onFocus={() => setF(true)} onBlur={() => setF(false)} style={{ width: '100%', height: 42, borderRadius: 9, outline: 'none',
...
function GcSelect({ style, ...p }: React.SelectHTMLAttributes<HTMLSelectElement>) {
  const [f, setF] = useState(false);
  return <select {...p} onFocus={() => setF(true)} onBlur={() => setF(false)} style={{ width: '100%', height: 42, borderRadius: 9, outline: 'none',
    border: `1.5px solid ${f ? C.blue : C.border}`, background: f ? C.white : C.bg,
    boxShadow: f ? '0 0 0 3px rgba(67,97,238,.1)' : 'none', padding: '0 13px',
    fontFamily: font, fontSize: 13.5, color: C.tx1, appearance: 'none', cursor: 'pointer',
...
function GcBtn({ variant = 'ghost', children, disabled, onClick, style, fw }: {
  ...
  const v = {
    ...
    primary: { background: h ? C.red : C.tx1, color: '#fff', transform: h ? 'translateY(-1px)' as const : 'none', boxShadow: h ? '0 4px 14px rgba(200,16,46,.28)' : 'none' },
  ...
  return (
    <button onClick={onClick} onMouseOver={() => setH(true)} onMouseOut={() => setH(false)}
      style={{ height: 32, padding: '0 14px', borderRadius: 8, fontFamily: font, fontSize: 12.5,
        ...
        background: h ? C.blueL : C.white, color: h ? C.blue : C.tx2,
...
function Modal({ children, onClose, maxW = 520 }: { children: React.ReactNode; onClose: () => void; maxW?: number }) {
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }} style={{
      ...
      <style>{`@keyframes gcFadeIn{from{opacity:0}to{opacity:1}}@keyframes gcRise{from{opacity:0;transform:translateY(18px) scale(.97)}to{opacity:1;transform:translateY(0) scale(1)}}`}</style>
    </div>
  );
}
...
function MHead({ eyebrow, title, onClose }: { eyebrow: string; title: string; onClose: () => void }) {
  ...
  return (
    <div style={{ padding: '24px 28px 18px', borderBottom: `1px solid ${C.border2}`,
      ...
      <button onClick={onClose} onMouseOver={() => setH(true)} onMouseOut={() => setH(false)}
...
function MFoot({ children }: { children: React.ReactNode }) {
  return <div style={{ padding: '16px 28px 24px', borderTop: `1px solid ${C.border2}`,
    display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, flexShrink: 0 }}>{children}</div>;
}

function SL({ children }: { children: React.ReactNode }) {
  return <p style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase',
    color: C.tx3, marginBottom: 14, paddingBottom: 10, borderBottom: `1px solid ${C.border2}` }}>{children}</p>;
}
      ...
      <MBody>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 18px',
          ...
          <span style={{ padding: '5px 12px', borderRadius: 99, fontSize: 11, fontWeight: 700,
            background: tier.bg, color: tier.color, border: `1.5px solid ${tier.ring}` }}>{user.tier}</span>
        </div>
...
function EditMemberModal({ user, onClose, onSaved }: { user: UserWithUid; onClose: () => void; onSaved: (u: Partial<UserWithUid>) => void }) {
  const [form, setForm] = useState({
    name: user.name ?? '', tier: user.tier ?? 'Silver',
    currentPoints: user.currentPoints ?? 0, lifetimePoints: user.lifetimePoints ?? 0,
    role: user.role ?? 'member', phoneNumber: user.phoneNumber ?? '',
  });
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <MBody>
        ...
        {error && <div style={{ padding: '10px 14px', background: C.redBg, border: '1px solid #FECDD3', borderRadius: 9, fontSize: 12.5, color: '#B42318' }}>{error}</div>}
      </MBody>
      <MFoot>
...
function EditStaffModal({ staff, storeIds, onClose, onSaved }: { staff: StaffWithUid; storeIds: string[]; onClose: () => void; onSaved: (u: Partial<StaffWithUid>) => void }) {
  const [form, setForm] = useState({
    name: staff.name ?? '', role: staff.role ?? 'cashier',
    storeLocation: staff.storeLocation ?? '', isActive: staff.isActive ?? true,
  });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
      ...
      <MBody>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderRadius: 12, background: C.bg, border: `1.5px solid ${C.border}` }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 99, background: form.isActive ? C.greenBg : C.border2, color: form.isActive ? '#027A48' : C.tx3 }}>
        ...
        {error && <div style={{ marginTop: 14, padding: '10px 14px', background: C.redBg, border: '1px solid #FECDD3', borderRadius: 9, fontSize: 12.5, color: '#B42318' }}>{error}</div>}
      </MBody>
      <MFoot>
        <GcBtn variant="ghost" onClick={onClose}>Batal</GcBtn>
        <GcBtn variant="primary" onClick={save} disabled={loading}>{loading ? 'Menyimpan…' : 'Simpan'}</GcBtn>
...
function CreateModal({ storeIds, onClose }: { storeIds: string[]; onClose: () => void }) {
  const [type, setType] = useState<'member'|'staff'>('member');
  const [form, setForm] = useState({
    name: '', email: '', phoneNumber: '', tier: 'Silver', role: 'member',
    staffRole: 'cashier', storeLocation: storeIds[0] ?? '', password: '', confirm: '',
  });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  ...
  async function create() {
    ...
    try {
      ...
      if (!r.ok) throw new Error('Gagal membuat akun.');
      onClose();
    } catch(e) { setError(String(e)); }
    finally { setLoading(false); }
  }
      ...
      <MBody>
        ...
        {type === 'member' ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 22 }}>
            ...
            <div>
              <FL>Tier Awal</FL>
              <GcSelect value={form.tier} onChange={e => setForm(p => ({ ...p, tier: e.target.value }))}>
                {['Silver','Gold','Platinum'].map(t => <option key={t}>{t}</option>)}
              </GcSelect>
            </div>
          </div></>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <GcInput type="password" placeholder="Ulangi password" value={form.confirm}
              onChange={e => setForm(p => ({ ...p, confirm: e.target.value }))}
              style={{ borderColor: form.confirm && form.password !== form.confirm ? '#F04438' : undefined }}/>
...
// ── Table Row Components (extracted to allow useState per-row) ────────────────
// ⚠️ CRITICAL: hooks (useState) cannot be called inside .map() loops.
// Each row must be its own component so React can track hooks correctly.

function UserRow({ u, isLast, onDetail, onEdit }: {
  ...
  return (
    <tr
      ...
      style={{ borderBottom: isLast ? 'none' : `1px solid ${C.border2}`, background: hovered ? '#F8F9FC' : C.white, transition: 'background .1s', cursor: 'pointer' }}
      onClick={onDetail}
    >
      ...
      <td style={{ padding: '14px 20px' }}>
        <span style={{ padding: '4px 11px', borderRadius: 99, fontSize: 11, fontWeight: 700, background: tier.bg, color: tier.color, border: `1.5px solid ${tier.ring}` }}>{u.tier}</span>
      </td>
...
function StaffRow({ s, isLast, onEdit }: {
  ...
  return (
    <tr
      onMouseOver={() => setHovered(true)}
      onMouseOut={() => setHovered(false)}
      style={{ borderBottom: isLast ? 'none' : `1px solid ${C.border2}`, background: hovered ? '#F8F9FC' : C.white, transition: 'background .1s' }}
    >
      <td style={{ padding: '14px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <td style={{ padding: '14px 20px' }}>
        <code style={{ fontSize: 12, background: C.blueL, padding: '3px 9px', borderRadius: 6, color: C.blue, border: `1px solid rgba(67,97,238,.15)` }}>{s.storeLocation || '—'}</code>
      </td>
      <td style={{ padding: '14px 20px' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 11px', borderRadius: 99, fontSize: 11, fontWeight: 700, letterSpacing: '.05em', background: s.isActive ? C.greenBg : C.border2, color: s.isActive ? '#027A48' : C.tx3 }}>
...
export default function MembersClient({ initialUsers, initialStaff, storeIds }: {
  ...
  const fUsers = useMemo(() => users.filter(u => {
    ...
    return (!q || u.name?.toLowerCase().includes(q) || u.email?.toLowerCase().includes(q) || u.phoneNumber?.includes(q))
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1400, fontFamily: font, WebkitFontSmoothing: 'antialiased' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: C.tx2, marginTop: 5 }}>Kelola member, tier, poin, dan akses staff.</p>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ display: 'flex', background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: 3 }}>
            {([{ k: 'member', l: 'Member', c: fUsers.length }, { k: 'staff', l: 'Staff', c: fStaff.length }] as const).map(t => (
              <button key={t.k} onClick={() => { setTab(t.k as any); setSearch(''); }}
                ...
                {t.l}
                <span style={{ padding: '1px 7px', borderRadius: 99, fontSize: 11, fontWeight: 700, background: tab === t.k ? C.blueL : C.border2, color: tab === t.k ? C.blue : C.tx3 }}>{t.c}</span>
              </button>
            ))}
          </div>
          {tab === 'member' && (
            <div style={{ display: 'flex', background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: 3 }}>
              {['All','Platinum','Gold','Silver'].map(t => (
                <button key={t} onClick={() => setTierF(t)}
                  style={{ padding: '5px 14px', borderRadius: 7, border: 'none', fontFamily: font, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', transition: 'all .13s', background: tierF === t ? C.white : 'transparent', color: tierF === t ? C.tx1 : C.tx3, boxShadow: tierF === t ? C.shadow : 'none' }}>
                  {t}
                </button>
              ))}
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 40, padding: '0 13px', minWidth: 240, background: C.white, border: `1.5px solid ${sf ? C.blue : C.border}`, borderRadius: 10, transition: 'all .14s', boxShadow: sf ? '0 0 0 3px rgba(67,97,238,.1)' : 'none' }}>
          ...
          <input style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontFamily: font, fontSize: 13.5, color: C.tx1 }}
        ...
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, boxShadow: C.shadow, overflow: 'hidden' }}>
          {fUsers.length === 0 ? (
            <div style={{ padding: '60px 24px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 13, color: C.tx3 }}>{search || tierF !== 'All' ? 'Tidak ada yang cocok dengan filter.' : 'Belum ada member terdaftar.'}</p>
            </div>
          ) : (
        ...
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, boxShadow: C.shadow, overflow: 'hidden' }}>
          {fStaff.length === 0 ? (
            <div style={{ padding: '60px 24px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 13, color: C.tx3 }}>{search ? 'Tidak ada yang cocok.' : 'Belum ada staff terdaftar.'}</p>
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: font }}>
              <thead>
      ...
      {editUser && (
        <EditMemberModal
          ...
          onSaved={u => { setUsers(p => p.map(x => x.uid === editUser.uid ? { ...x, ...u } : x)); setEditUser(null); }}
        />
      )}
      {editStaff && (
        <EditStaffModal
          ...
          onSaved={u => { setStaff(p => p.map(x => x.uid === editStaff.uid ? { ...x, ...u } : x)); setEditStaff(null); }}
        />
      )}
      {showCreate && <CreateModal storeIds={storeIds} onClose={() => setShowCreate(false)}/>}
    </div>
  );
}