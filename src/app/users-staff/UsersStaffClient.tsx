  ...
  return (
    <div className="p-8 max-w-[1400px]">
      <div className="flex items-center justify-between mb-8">
        ...
        <button className="px-4 py-2.5 rounded-xl text-white text-sm font-semibold shadow-card-blue" style={{ background: "linear-gradient(135deg,#4361EE,#3A0CA3)" }} onClick={() => setShowAccountModal(true)}>
          + Tambah Akun
        </button>
      </div>
      ...
      {showAccountModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-8 shadow-lg w-[400px]">
            <h2 className="font-bold text-lg mb-4">Tambah Akun</h2>
            <div className="mb-4">
              <label className="block text-sm font-medium mb-1">Tipe Akun</label>
              <select
                className="w-full border border-border rounded-xl px-3 py-2 text-sm text-tx1 outline-none focus:border-blue1 transition-colors"
                value={accountType}
                onChange={e => setAccountType(e.target.value as 'member' | 'staff')}
              >
            ...
            {accountType === 'member' ? (
              <form className="space-y-3">
                <div>
                  <label className="block text-xs font-medium mb-1">Nama Member</label>
                  <input className="w-full border border-border rounded-xl px-3 py-2 text-sm text-tx1 outline-none focus:border-blue1 transition-colors" placeholder="Nama" />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1">Email</label>
                  <input className="w-full border border-border rounded-xl px-3 py-2 text-sm text-tx1 outline-none focus:border-blue1 transition-colors" placeholder="Email" />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1">No. HP</label>
                  <input className="w-full border border-border rounded-xl px-3 py-2 text-sm text-tx1 outline-none focus:border-blue1 transition-colors" placeholder="No. HP" />
                </div>
                {/* Add more member fields as needed */}
                <button type="submit" className="w-full mt-2 px-4 py-2 bg-blue-600 text-white rounded">Buat Member</button>
              </form>
            ) : (
              <form className="space-y-3">
                <div>
                  <label className="block text-xs font-medium mb-1">Nama Staff/Admin</label>
                  <input className="w-full border border-border rounded-xl px-3 py-2 text-sm text-tx1 outline-none focus:border-blue1 transition-colors" placeholder="Nama" />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1">Email</label>
                ...
                {/* Add more staff fields as needed */}
                <button type="submit" className="w-full mt-2 px-4 py-2 bg-blue-600 text-white rounded">Buat Staff/Admin</button>
              </form>
            )}
            <button className="mt-4 px-4 py-2 bg-gray-200 text-tx2 rounded w-full" onClick={() => setShowAccountModal(false)}>Tutup</button>
          </div>
        </div>
      )}
      {/* Header */}
