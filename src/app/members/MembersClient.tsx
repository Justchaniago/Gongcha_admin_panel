  ...
  return (
    <div className="p-8 max-w-[1400px]">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          className="px-4 py-2.5 rounded-xl text-white text-sm font-semibold shadow-card-blue"
          style={{ background: "linear-gradient(135deg,#4361EE,#3A0CA3)" }}
          onClick={() => {
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(15,23,42,0.4)", backdropFilter: "blur(4px)" }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: "1px solid #F1F5F9" }}>
              <h2 className="font-display font-bold text-tx1 text-lg">Tambah Akun Baru</h2>
              <button onClick={() => setShowCreateModal(false)} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-s2 transition-colors text-tx3">✕</button>
            </div>
            <div className="px-6 py-5 space-y-4">
              <div className="flex gap-2 mb-2">
                <button
                  className={`flex-1 px-3 py-2 rounded-lg text-sm font-semibold border ${accountType === 'member' ? 'bg-blue1 text-white' : 'bg-s2 text-tx2'}`}
                  onClick={() => { setAccountType('member'); setForm({}); setErrorCreate(""); }}
                >Member</button>
                <button
                  className={`flex-1 px-3 py-2 rounded-lg text-sm font-semibold border ${accountType === 'staff' ? 'bg-blue1 text-white' : 'bg-s2 text-tx2'}`}
                  onClick={() => { setAccountType('staff'); setForm({}); setErrorCreate(""); }}
                >Staff/Admin</button>
              </div>
              {/* Dynamic Form */}
              {accountType === 'member' ? (
                <>
                  <div>
                    <label className="text-xs font-medium text-tx2 block mb-1">Nama</label>
                    <input type="text" className="w-full border border-border rounded-xl px-3 py-2 text-sm text-tx1 outline-none focus:border-blue1 transition-colors"
                      value={form.name || ""}
                      onChange={e => setForm((f: any) => ({ ...f, name: e.target.value }))}
                    />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-tx2 block mb-1">No. HP</label>
                    <input type="text" className="w-full border border-border rounded-xl px-3 py-2 text-sm text-tx1 outline-none focus:border-blue1 transition-colors"
                  ...
                  <div>
                    <label className="text-xs font-medium text-tx2 block mb-1">Tier</label>
                    <select className="w-full border border-border rounded-xl px-3 py-2 text-sm text-tx1 outline-none focus:border-blue1 transition-colors"
                      value={form.tier || "Silver"}
                      onChange={e => setForm((f: any) => ({ ...f, tier: e.target.value }))}
                    >
                      {['Silver', 'Gold', 'Platinum'].map(t => <option key={t}>{t}</option>)}
                    </select>
                  </div>
                ...
                <>
                  ...
                  <div>
                    ...
                    <input type="text" className="w-full border border-border rounded-xl px-3 py-2 text-sm text-tx1 outline-none focus:border-blue1 transition-colors"
                      ...
                      onChange={e => setForm((f: any) => ({ ...f, storeLocation: e.target.value }))}
                    />
                  </div>
                </>
              )}
              {errorCreate && <p className="text-xs text-red-500">{errorCreate}</p>}
            </div>
            <div className="px-6 py-4 flex gap-2 justify-end" style={{ borderTop: "1px solid #F1F5F9" }}>
              <button onClick={() => setShowCreateModal(false)} className="px-4 py-2 rounded-xl text-sm text-tx2 border border-border hover:bg-s2 transition-colors">Batal</button>
              <button
                className="px-4 py-2 rounded-xl text-sm text-white font-semibold transition-all disabled:opacity-50"
                style={{ background: "linear-gradient(135deg,#4361EE,#3A0CA3)" }}
                ...
                onClick={async () => {
                  ...
                  setErrorCreate("");
                  try {
                    // TODO: Implement API call for creating member/staff
                    setShowCreateModal(false);
                  } catch (e) {
                    setErrorCreate(String(e));
                  } finally {
                    setLoadingCreate(false);
                  }
                }}
              >{loadingCreate ? "Menyimpan..." : "Simpan"}</button>
            </div>
          </div>
        </div>
      )}
