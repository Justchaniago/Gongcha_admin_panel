...
import {
  Menu, X, LayoutDashboard, Clock, Receipt, BarChart2,
  ChevronRight, LogOut, Home, Users, Store, Star, Bell,
  Settings, SlidersHorizontal, ChevronDown, Calendar,
  TrendingUp, CheckCircle2, XCircle, AlertCircle, ArrowUpRight,
  Activity, FileText, Zap, CreditCard,
} from "lucide-react";
...
function useCounter(target: number, duration = 600) {
  ...
  useEffect(() => {
    const c = animate(v, target, { duration: duration / 1000, ease: "easeOut", onUpdate: x => set(Math.round(x)) });
    return c.stop;
  }, [target]);
  return v;
}
...
// ─────────────────────────────────────────────────────────────────
// DESIGN TOKENS  (single source of truth, no Tailwind classes for spacing/color)
// ─────────────────────────────────────────────────────────────────
const T = {
  bg:        "#F4F5F7",
  surface:   "#FFFFFF",
...
const Drawer = ({ open, onClose, userName, role, router }: {
  open: boolean; onClose: () => void; userName: string; role: string; router: any;
}) => (
  <AnimatePresence>
    {open && (
      <>
        <motion.div
          ...
          style={{ position: "fixed", inset: 0, zIndex: 40, background: "rgba(0,0,0,0.32)", backdropFilter: "blur(4px)" }}
          onClick={onClose}
        />
        <motion.div
          ...
          <div style={{ padding: "56px 20px 16px", borderBottom: `1px solid ${T.border}` }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#F9FAFB", borderRadius: 12, padding: "10px 12px" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: T.blueL, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <span style={{ color: T.blueD, fontSize: 12, fontWeight: 800 }}>{userName[0]?.toUpperCase()}</span>
              </div>
            ...
            {NAV.map(({ icon: Icon, label, href }) => {
              ...
              return (
                ...
                >
                  ...
                  {active && <ChevronRight size={13} style={{ marginLeft: "auto", color: T.blue }} />}
                </button>
              );
            })}
          </div>

          <div style={{ padding: "8px 10px 32px", borderTop: `1px solid ${T.border}` }}>
            <button style={{
              width: "100%", display: "flex", alignItems: "center", gap: 10,
      ...
      <>
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
            ...
            <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: "50%", background: T.border, border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
              <X size={13} color={T.tx3} />
            </button>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            {[{ v: "realtime", label: "Real-time" }, { v: "range", label: "Date Range" }].map(({ v, label }) => (
              <button key={v} onClick={() => setMode(v)} style={{
                flex: 1, padding: "10px", borderRadius: 12, border: `1.5px solid ${mode === v ? T.blue : T.border2}`,
                background: mode === v ? T.blueL : "#F9FAFB", color: mode === v ? T.blueD : T.tx3,
                ...
                {[{ lbl: "From", val: dateFrom, set: setDateFrom }, { lbl: "To", val: dateTo, set: setDateTo }].map(({ lbl, val, set }) => (
                  <div key={lbl} style={{ flex: 1 }}>
                    ...
                    <input type="date" value={val} onChange={e => set(e.target.value)} style={{
                      width: "100%", background: "#F9FAFB", border: `1px solid ${T.border2}`,
                      borderRadius: 10, padding: "8px 10px", fontSize: 12, fontWeight: 500, color: T.tx1,
                    }} />
                  </div>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ position: "relative" }}>
                <select value={storeId} onChange={e => setStoreId(e.target.value)} style={{
                  width: "100%", appearance: "none", background: "#F9FAFB", border: `1px solid ${T.border2}`,
                  borderRadius: 10, padding: "10px 32px 10px 12px", fontSize: 12, fontWeight: 500, color: T.tx1,
                }}>
                  <option value="all">All Stores</option>
                  {stores.map((s: StoreItem) => <option key={s.uid ?? s.id} value={s.uid ?? s.id}>{s.name}</option>)}
                </select>
                <ChevronDown size={13} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: T.tx4, pointerEvents: "none" }} />
              </div>
            </div>
          )}
          ...
          <button onClick={onClose} style={{
            ...
            background: T.navy, color: "#fff", fontSize: 13, fontWeight: 800, letterSpacing: ".02em",
          }}>Apply</button>
        </motion.div>
      </>
    )}
...
// ─────────────────────────────────────────────────────────────────
const QueueCard = ({ tx, onApprove, onReject, idx }: {
  tx: any; onApprove: (id: string, mid: string, pts: number) => void;
  onReject: (id: string) => void; idx: number;
}) => {
  ...
  const time = tx.createdAt?.toDate
    ? new Date(tx.createdAt.toDate()).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })
    : "—";
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 18px", pointerEvents: "none" }}>
        <motion.div style={{ opacity: appOp, display: "flex", alignItems: "center", gap: 6, color: T.green }}>
          <CheckCircle2 size={15} strokeWidth={2.5} />
          <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: ".08em" }}>APPROVE</span>
        </motion.div>
        <motion.div style={{ opacity: rejOp, display: "flex", alignItems: "center", gap: 6, color: T.red }}>
          <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: ".08em" }}>REJECT</span>
      ...
      <motion.div
        ...
        onDragEnd={(_, { offset }) => {
          if      (offset.x >  110) approve();
          else if (offset.x < -110) reject();
        ...
        {busy ? (
          ...
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: 10, color: T.tx4, marginTop: 2, display: "flex", alignItems: "center", gap: 3 }}>
                  <FileText size={9} />
                  {tx.posTransactionId ?? tx.receiptNumber ?? "—"}
                </p>
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10, paddingTop: 10, borderTop: `1px solid ${T.border}` }}>
              <span style={{ fontSize: 10, color: T.tx4, display: "flex", alignItems: "center", gap: 4 }}>
                <Clock size={10} /> {time}
              </span>
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={reject} style={{
                  ...
                  fontSize: 10, fontWeight: 700, color: T.tx3, cursor: "pointer",
                }}>
...
// ─────────────────────────────────────────────────────────────────
const StatCard = ({ label, value, valueColor, icon: Icon, iconBg, iconColor, badge, onClick, delay = 0 }: {
  label: string; value: string | number; valueColor?: string;
  icon: React.ElementType; iconBg: string; iconColor: string;
  badge?: string; onClick?: () => void; delay?: number;
}) => (
  <motion.button
    ...
    style={{
      ...
      borderRadius: T.r16, padding: "14px 14px 12px",
  ...
  >
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
      ...
      {badge && (
        <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 7px", borderRadius: 99, background: iconBg, color: iconColor, letterSpacing: ".04em" }}>
          {badge}
        </span>
      )}
    </div>
...
export default function DashboardMobile({ initialRole, initialTransactions, initialUsers, initialStores }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    if (!isAdmin && assignedId) pq = query(collection(db, "transactions"),
      where("storeId", "==", assignedId), where("createdAt", ">=", today), orderBy("createdAt", "desc"));

    const unPending = onSnapshot(pq, snap => {
  ...
  useEffect(() => {
    ...
    const tid = isAdmin
      ? (effectiveStore === "all" ? `${today}_GLOBAL` : `${today}_${effectiveStore}`)
      : (assignedId ? `${today}_${assignedId}` : null);
  ...
  const filteredTx = useMemo(() =>
    effectiveStore === "all" ? allTx : allTx.filter(t => t.storeId === effectiveStore),
  [allTx, effectiveStore]);

  const pendingQueue = useMemo(() =>
    rawPending
      .filter(t => (t.type === "EARN" || t.type === "earn") && (t.status === "PENDING" || t.status === "pending"))
  ...
  const cancelCount  = filteredTx.filter(t => t.status === "CANCELLED").length;
  const claimsCount  = pendingCount + cancelCount;
  ...
  const storePerf = useMemo(() => {
    ...
    const max = Math.max(...dailyStats.map(d => d.totalRevenue ?? 0), 1);
    return [...dailyStats].sort((a, b) => a.date.localeCompare(b.date)).slice(-5).map(d => ({
      name: d.date, pct: Math.round(((d.totalRevenue ?? 0) / max) * 100), rev: d.totalRevenue ?? 0,
    }));
  }, [dailyStats]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", background: T.bg, fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', sans-serif", WebkitFontSmoothing: "antialiased", overflowX: "hidden" }}>
      ...
      <Drawer open={drawerOpen} onClose={() => setDrawer(false)} userName={userName} role={role} router={router} />
      <FilterSheet
        open={filterOpen} onClose={() => setFilter(false)}
        mode={mode} setMode={setMode}
        dateFrom={dfrom} setDateFrom={setDfrom}
        dateTo={dto} setDateTo={setDto}
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: T.tx1, letterSpacing: "-.01em" }}>{greeting}, {userName.split(" ")[0]}</p>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 5, marginTop: 3 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#10B981", display: "block", animation: "pulse 2s infinite" }} />
        ...
        <button
          ...
          style={{ width: 36, height: 36, borderRadius: 11, border: `1px solid ${filterActive ? T.blue : T.border2}`, background: filterActive ? T.blueL : T.surface, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}
        >
      ...
      {/* ── REVENUE HERO  (always visible, above tab content) ── */}
      <motion.div
        initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.32 }}
        style={{ flexShrink: 0, background: T.surface, borderBottom: `1px solid ${T.border}` }}
      >
        <button
          onClick={() => router.push("/transactions")}
          style={{
            width: "100%", margin: "0", padding: "14px 16px 16px",
            background: T.navy2, border: "none", cursor: "pointer", textAlign: "left",
          }}
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
            ...
            <div style={{ width: 26, height: 26, borderRadius: 8, background: "rgba(255,255,255,.08)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <ArrowUpRight size={13} color="rgba(255,255,255,.5)" />
            </div>
          </div>
          <p style={{ fontSize: 30, fontWeight: 900, color: "#fff", letterSpacing: "-.035em", lineHeight: 1 }}>{fmt(cRev)}</p>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10 }}>
            <span style={{ fontSize: 9, fontWeight: 800, padding: "3px 8px", borderRadius: 99, background: "rgba(16,185,129,.18)", color: "#6EE7B7", border: "1px solid rgba(16,185,129,.25)" }}>
              ↑ Verified only
            </span>
          ...
          >
                ...
                <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r16, overflow: "hidden" }}>
                  ...
                  {recentTrx.slice(0, 3).length === 0 ? (
                    <div style={{ padding: "20px 0", textAlign: "center" }}>
                      <p style={{ fontSize: 12, color: T.tx4 }}>No transactions yet</p>
                    </div>
            ...
            {tab === "queue" && (
              <div>
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 3, color: T.blue }}>
                    <XCircle size={12} strokeWidth={2.5} />
                    <span style={{ fontSize: 10, fontWeight: 700 }}>Swipe left to reject</span>
                  </div>
                  <span style={{ color: T.border2, fontSize: 10 }}>·</span>
                  <div style={{ display: "flex", alignItems: "center", gap: 3, color: T.green }}>
                    <CheckCircle2 size={12} strokeWidth={2.5} />
                    <span style={{ fontSize: 10, fontWeight: 700 }}>Swipe right to approve</span>
                  </div>
                </motion.div>
                ...
                {loading ? (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 0", gap: 10 }}>
                    <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }}>
                      <Activity size={18} color={T.border2} />
                    </motion.div>
                  ...
                  <AnimatePresence mode="popLayout">
                    {pendingQueue.length > 0 ? (
                      <>
                        ...
                        {pendingQueue.map((tx, i) => (
                          <QueueCard key={tx.id} tx={tx} idx={i} onApprove={handleApprove} onReject={handleReject} />
                        ))}
                      </>
                    ) : (
                      <motion.div key="empty" initial={{ opacity: 0, scale: .97 }} animate={{ opacity: 1, scale: 1 }}
                        style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 0", background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r16 }}
                      >
            ...
            {tab === "transactions" && (
              <div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  ...
                  <button onClick={() => router.push("/transactions")} style={{ fontSize: 10, fontWeight: 700, color: T.blue, background: T.blueL, border: "none", borderRadius: 99, padding: "4px 10px", cursor: "pointer" }}>
                    View all
                  </button>
                </div>
                {(mode === "range" && dfrom && dto) && (
                ...
                <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r16, overflow: "hidden" }}>
                  {recentTrx.length === 0 ? (
                    <div style={{ padding: "40px 0", textAlign: "center" }}>
                      <Receipt size={24} color={T.border2} style={{ margin: "0 auto 8px" }} />
                      <p style={{ fontSize: 12, color: T.tx4 }}>No transactions found</p>
                    </div>
                  ) : recentTrx.map((tx, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          <p style={{ fontSize: 12, fontWeight: 800, color: tx.type === "redeem" ? "#7C3AED" : T.tx1 }}>
                            {tx.type === "redeem" ? "Redeem" : fmt(tx.amount)}
                          </p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                {isAdmin && storePerf.length > 0 && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .1 }}
                    style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r16, padding: "14px 14px" }}
                  >
      ...
      <div style={{ flexShrink: 0, display: "flex", justifyContent: "center", padding: "8px 0 24px", background: T.bg }}>
        <div style={{
          display: "flex", alignItems: "center", gap: 4,
          background: T.navy2, borderRadius: 99,
          padding: "5px 6px",
          boxShadow: "0 8px 24px rgba(0,0,0,.22), 0 2px 8px rgba(0,0,0,.12)",
        }}>
          {TABS.map(({ id, icon: Icon, label }) => {
            ...
            return (
              <button
                ...
                style={{
                  position: "relative",
                  display: "flex", alignItems: "center", justifyContent: "center", gap: active ? 6 : 0,
                  height: 36,
                  padding: active ? "0 14px" : "0 14px",
                  borderRadius: 99,
                  border: "none",
