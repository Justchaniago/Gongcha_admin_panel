...
function GcBtn({ variant = 'ghost', children, disabled, onClick, style, fw }: {
  variant?: 'ghost' | 'primary' | 'blue' | 'danger'; children: React.ReactNode;
  ...
  const v = {
    ...
    danger:  { background: h ? '#B91C1C' : C.red, color: '#fff', boxShadow: h ? '0 4px 14px rgba(200,16,46,.4)' : 'none' },
  };
  return (
...
// ── Modals ────────────────────────────────────────────────────────────────────
function MemberDetailModal({ user, onClose, onEdit, onDelete }: { user: UserWithUid; onClose: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <Modal onClose={onClose} maxW={540}>
      ...
      <MFoot>
        <GcBtn variant="danger" onClick={onDelete} style={{ marginRight: 'auto' }}>Hapus Member</GcBtn>
...
// ── Table rows ─────────────────────────────────────────────────────────────────
function UserRow({ u, isLast, onDetail, onEdit, onDelete }: { u: UserWithUid; isLast: boolean; onDetail: () => void; onEdit: () => void; onDelete: () => void }) {
...
function StaffRow({ s, isLast, onEdit, onDelete }: { s: StaffWithUid; isLast: boolean; onEdit: () => void; onDelete: () => void }) {
  const [hov, setHov] = useState(false);
...
export default function MembersClient({ initialUsers, initialStaff, storeIds }: {
  ...
  const [deleteUser,  setDeleteUser]  = useState<UserWithUid  | null>(null);
  const [deleteStaff, setDeleteStaff] = useState<StaffWithUid | null>(null);
  ...
  async function handleDeleteUser() {
    ...
    try {
      const r = await fetch(`/api/members/${deleteUser.uid}`, { method: 'DELETE' });
      const d = await r.json();
      if (!r.ok) throw new Error(d.message ?? 'Gagal menghapus.');
  ...
  async function handleDeleteStaff() {
    ...
    try {
      const r = await fetch(`/api/staff/${deleteStaff.uid}`, { method: 'DELETE' });
      const d = await r.json();
      if (!r.ok) throw new Error(d.message ?? 'Gagal menghapus.');
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1400, fontFamily: font, WebkitFontSmoothing: 'antialiased' }}>
        ...
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, boxShadow: C.shadow, overflow: 'hidden' }}>
          {fUsers.length === 0 ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: font }}>
              ...
              <tbody>
                {fUsers.map((u, i) => (
                  <UserRow key={u.uid} u={u} isLast={i === fUsers.length - 1}
        ...
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, boxShadow: C.shadow, overflow: 'hidden' }}>
          {fStaff.length === 0 ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: font }}>
              ...
              <tbody>
                {fStaff.map((s, i) => (
                  <StaffRow key={s.uid} s={s} isLast={i === fStaff.length - 1}
      ...
      {deleteUser && (
        <ConfirmDialog
          title="Hapus Member?"
          message={`Member "${deleteUser.name}" dan akun Firebase Auth-nya akan dihapus permanen.`}
      ...
      {deleteStaff && (
        <ConfirmDialog
          title="Hapus Staff?"
          message={`Staff "${deleteStaff.name}" dan akun Firebase Auth-nya akan dihapus permanen.`}
